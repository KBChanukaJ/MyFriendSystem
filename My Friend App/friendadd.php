...
$query = "SELECT profile_name FROM friends WHERE profile_name <> ? AND friend_id NOT IN (SELECT friend_id2 FROM myfriends WHERE friend_id1 = ?) LIMIT ?, ?";
$stmt = $conn->prepare($query);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addFriend'])) {
    ...
    $friendProfileName = $_POST['friendProfileName'];
                                    ...
                                    <tbody>
                                        <?php
                                            ...
                                            foreach ($usersList as $user) {
                                                ...
                                                echo '<input type="hidden" name="friendProfileName" value="' . $user['profile_name'] . '">';
                                                echo '<button type="submit" name="addFriend" class="btn btn-outline-dark btn-sm rounded-0">Add as Friend</button>';
                                                echo '</td>';
                                                echo '</tr>';
                                            }
                                        ?>
