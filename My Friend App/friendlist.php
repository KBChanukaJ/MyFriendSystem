...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unfriend'])) {
    ...
    $friendId = isset($_POST['unfriend']) ? $_POST['unfriend'] : null;

    if ($friendId) {
        // Delete the friend record from myfriends table
        $deleteQuery = "DELETE FROM myfriends WHERE (friend_id1 = ? AND friend_id2 = ?) OR (friend_id1 = ? AND friend_id2 = ?)";
        $deleteStmt = $conn->prepare($deleteQuery);
        $deleteStmt->bind_param("iiii", $loggedInUserId, $friendId, $friendId, $loggedInUserId);
        $deleteStmt->execute();
        ...
        <?php foreach ($friendsListWithMutualCount as $friend) : ?>
            <tr>
                ...
                <td width="20%" class="p-3">
                    <input type="hidden" name="friendProfileName[<?php echo $friend['friend_id']; ?>]" value="<?php echo $friend['profile_name']; ?>">
                    <button type="submit" name="unfriend" class="btn btn-outline-dark btn-sm rounded-0" value="<?php echo $friend['friend_id']; ?>">Unfriend</button>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
